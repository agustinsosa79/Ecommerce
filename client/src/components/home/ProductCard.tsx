...
ProductCard.Button = ({children, onClick} : {children: ReactNode, onClick: MouseEventHandler<HTMLButtonElement>}) => (
    <button onClick={onClick} className="bg-black/80 backdrop-blur-md shadow-md text-white shadow-black absolute inset-0 m-auto text-sm font-sans w-90 cursor-pointer font-bold h-15 top-60 opacity-0 group-hover:opacity-100 hover:bg-white/80 hover:transition-all hover:duration-800 hover:text-black transition-opacity duration-75 rounded-4xl">
    {children}
  </button>
    )
...
ProductCard.Info = ({name, price}: {name: string, price: number}) => {
  ...
  return(
    <div className=" p-10 h-full  cursor-pointer hover:bg-black/10">
